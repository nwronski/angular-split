...
class SplitComponent {
     ...
     * @param {?} docRef
     */
    constructor(ngZone, elRef, cdRef, renderer, docRef /* Document */) {
        this.ngZone = ngZone;
        this.elRef = elRef;
        this.cdRef = cdRef;
    ...
    build(resetOrders, resetSizes) {
        ...
        let /** @type {?} */ containerSizePixel; // TODO: This is immediately overwritten
        ...
        if (percentToDispatch !== 0 && this.displayedAreas.length > 0) {
            const /** @type {?} */ areasNotZero = this.displayedAreas.filter(a => a.size !== 0);
            if (areasNotZero.length > 0) {
                const /** @type {?} */ areasWithoutDefecit = this.displayedAreas.filter(a => {
